...
import { itnwSubjectsData as subjectsData } from '../data/itNetworkingSubjects';
import { materialsData } from '../data/materials';

export default function ITNetworkingTutorials({ onNavigate }: { onNavigate: (view: string) => void, key?: string }) {
  ...
  return (
    <motion.div 
      ...
      className="min-h-[100dvh] bg-gradient-to-br from-[#f6f3ff] via-[#f2f6ff] to-[#e1efff] font-sans text-slate-800 flex flex-col relative selection:bg-indigo-200 w-full overflow-x-hidden"
    >
      <RoadmapHeader onNavigate={onNavigate} currentView="tutorials" track="itnw" />
        ...
        {!selectedSubject ? (
          ...
          >
            <div className="mt-8 mb-10 text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl font-black text-[#4f46e5] leading-tight mb-4" style={{ textShadow: "2px 2px 0px #c7d2fe" }}>
                IT-NW LEARNING MATERIALS
              </h1>
            ...
            <div className="w-full max-w-2xl mb-8 relative mx-auto lg:mx-0">
              ...
              <input
                type="text"
                className="block w-full pl-12 pr-6 py-4 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#4f46e5] text-slate-800 font-medium placeholder-slate-400 transition-all text-base"
            ...
            {filteredSubjects.length === 0 ? (
              ...
              >
                ...
                <button 
                  ...
                  className="mt-6 text-[#4f46e5] font-bold hover:underline"
                >
                  Clear search
                </button>
              </motion.div>
                ...
                {filteredSubjects.map((subject, index) => (
                  ...
                  >
                    <div>
                      ...
                      <h3 className="font-bold text-sm text-slate-800 leading-tight mb-2 group-hover:text-[#4f46e5] transition-colors">{subject.title}</h3>
                      <p className="text-slate-500 text-xs line-clamp-2">{subject.description}</p>
                    </div>
                    <div className="mt-4 flex items-center text-[#4f46e5] font-semibold text-xs opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0">
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  courseContent.map((material, index) => (
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <h4 className="font-semibold text-slate-800 group-hover:text-[#4f46e5] transition-colors line-clamp-1">{material.title}</h4>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs text-slate-500 uppercase tracking-wider">{material.type} {material.size && `• ${material.size}`}</span>
                          {material.dateAdded && <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full group-hover:bg-indigo-100 group-hover:text-[#4f46e5] transition-colors">{material.dateAdded}</span>}
                        </div>
                      </div>
                      <div className="p-2 text-slate-400 group-hover:text-[#4f46e5] group-hover:bg-indigo-100 rounded-lg transition-colors shrink-0">
              ...
              <motion.div 
                ...
                className="space-y-6"
              >
                <div className="bg-[#4f46e5] text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
                  ...
                  {syllabusMaterial ? (
                    <a href={syllabusMaterial.url || '#'} download className="bg-white text-[#4f46e5] font-bold px-5 py-2.5 rounded-xl w-full flex items-center justify-center gap-2 hover:bg-indigo-50 transition-colors relative z-10 shadow-sm">
                      <Download className="w-4 h-4" />
                      Download Syllabus ({syllabusMaterial.size || 'PDF'})
                    </a>
                  ) : (
                    <button disabled className="bg-indigo-800/50 text-indigo-200 font-bold px-5 py-2.5 rounded-xl w-full flex items-center justify-center gap-2 cursor-not-allowed relative z-10 shadow-inner">
                      Not Available Yet
                    </button>
                  )}
                </div>
