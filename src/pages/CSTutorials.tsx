        ...
        {!selectedSubject ? (
          ...
          >
            ...
            <div className="w-full max-w-2xl mb-8 relative mx-auto lg:mx-0">
              ...
              <input
                type="text"
                className="block w-full pl-12 pr-6 py-4 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-slate-800 font-medium placeholder-slate-400 transition-all text-base"
                ...
                {filteredSubjects.map((subject, index) => (
                  ...
                  >
                    <div>
                      ...
                      <h3 className="font-bold text-sm text-slate-800 leading-tight mb-2 group-hover:text-[#2563eb] transition-colors">{subject.title}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  courseContent.map((material, index) => (
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <h4 className="font-semibold text-slate-800 group-hover:text-[#2563eb] transition-colors line-clamp-1">{material.title}</h4>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs text-slate-500 uppercase tracking-wider">{material.type} {material.size && `• ${material.size}`}</span>
                          {material.dateAdded && <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">{material.dateAdded}</span>}
                        </div>
                      </div>
              ...
              >
                <div className="bg-[#2563eb] text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
                  ...
                  {syllabusMaterial ? (
                    <a href={syllabusMaterial.url || '#'} download className="bg-white text-[#2563eb] font-bold px-5 py-2.5 rounded-xl w-full flex items-center justify-center gap-2 hover:bg-blue-50 transition-colors relative z-10 shadow-sm">
                      <Download className="w-4 h-4" />
                      Download Syllabus ({syllabusMaterial.size || 'PDF'})
                    </a>
                  ) : (
                    <button disabled className="bg-blue-800/50 text-blue-200 font-bold px-5 py-2.5 rounded-xl w-full flex items-center justify-center gap-2 cursor-not-allowed relative z-10 shadow-inner">
                      Not Available Yet
                    </button>
                  )}
                </div>
