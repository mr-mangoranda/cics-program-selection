...
export default function ITNetworkingRoadmap({ onNavigate }: { onNavigate: (view: string) => void, key?: string }) {
  ...
  return (
    ...
    >
      ...
      <RoadmapHeader onNavigate={onNavigate} currentView="roadmap" track="itnw" />
      ...
      <main className="flex-grow flex flex-col items-center w-full max-w-5xl mx-auto px-4 z-10 relative pb-32">
        ...
        >
          <h1 
            className="text-5xl sm:text-6xl md:text-8xl font-black text-center text-[#4f46e5] leading-[0.95]"
            style={{ textShadow: "5px 5px 0px #c7d2fe" }}
          >
            <span className="block mb-2 md:mb-4">IT - NETWORKING</span>
          </h1>
        </motion.div>
            ...
            return (
              <React.Fragment key={sem.semesterName}>
                ...
                {sem.subjects.map((subjectKey, i) => {
                  ...
                  const subject = subjectsData[subjectKey] as SubjectDetails;
